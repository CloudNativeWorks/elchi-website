...
| `elchi-stack net-test [--mesh]` | Cross-node connectivity check from this node to every peer: per-service TCP reachability + connect latency, ICMP RTT/loss, and a path-MTU (DF) probe — catches MTU black holes and lossy links that plain TCP checks miss. `--mesh` (M1 + root) makes every node probe every other node for a full src×dst matrix. Options: `--timeout=3`, `--samples=3`, `--icmp-count=10`, `--json`. Exit 0 clean; 1 on FAIL / MTU black hole / ≥20% loss. |
